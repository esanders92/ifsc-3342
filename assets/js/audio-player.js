		...
		if (m.win && m.ie) {
			...
			if (y) {
				...
				if (AC) {
					...
					for (var z = 0; z < x; z++) {
						if (
							!(AC[z].nodeType == 1 && AC[z].nodeName == "PARAM") &&
							!(AC[z].nodeType == 8)
						) {
	...
	return {
		setup: function (J, I) {
			...
			if (audioplayer_swfobject.hasFlashPlayerVersion(G)) {
				...
				audioplayer_swfobject.createCSS(
					"p.audioplayer_container span",
					"visibility:hidden;height:24px;overflow:hidden;padding:0;border:none;"
				);
			}
		},
		...
		embed: function (I, K) {
			...
			O.playerID = I;
			audioplayer_swfobject.embedSWF(
				F,
				I,
				N.width.toString(),
				"24",
